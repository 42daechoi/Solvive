...
public class SpawnManager : MonoBehaviourPun
{
    ...
    [PunRPC]
    void RequestSpawnSync(int actorNumber, PhotonMessageInfo info)
    {
        photonView.RPC("SyncUsedSpawnPoints", info.Sender, isSpawned);
    }

    [PunRPC]
    void SyncUsedSpawnPoints(bool[] used)
    {
        isSpawned = used;
        spawnInitialized = true;
        SpawnPlayers();
    ...
    {
        int spawnIdx = detectorIndex switch
        {
            0 => 0,
            >= 1 and <= 3 => 1,
            4 => 2,
